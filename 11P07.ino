...
void loop() {
  ...
  if ((dist_raw == 0.0) || (dist_raw > _DIST_MAX)) {
    ...
    dist_prev = dist_filtered;// Cut higher than maximum
    digitalWrite(PIN_LED, 1);       // LED OFF
  } else if (dist_raw < _DIST_MIN) {
    dist_filtered = dist_prev;
    dist_prev = dist_filtered;       // cut lower than minimum
    digitalWrite(PIN_LED, 1);       // LED OFF
  ...
  dist_ema = 0.5 * dist_filtered + 0.5 * dist_ema;

  // adjust servo position according to the USS read value
  // add your code here!
  float _DUTY_INTERVAL = (_DUTY_MAX-_DUTY_MIN)/180;
  if (dist_ema <= _TARGET_LOW){
    myservo.writeMicroseconds(_DUTY_MIN);
  }
  else if(dist_ema >_TARGET_LOW && dist_ema<= _TARGET_HIGH){
   myservo.writeMicroseconds((_DUTY_INTERVAL*(dist_ema-180))+_DUTY_MIN);
  }
